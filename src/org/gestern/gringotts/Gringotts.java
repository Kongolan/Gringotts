...
import org.gestern.gringotts.commands.ReloadCommand;
...
public class Gringotts extends JavaPlugin {
	...
	/**
	 * The account holder factory is the place to go if you need an
	 * AccountHolder instance for an id.
	 */
	...
	public void onEnable() {
		...
		try {
			...
			// just call DAO once to ensure it's loaded before startup is
			// complete
			dao = getDAO();

			// load and init configuration
			saveDefaultConfig(); // saves default configuration if no config.yml
									// exists yet
			reloadConfig();
	...
	private void registerCommands() {
		getCommand("gringotts").setExecutor(new ReloadCommand(this));
	}
	...
	private void registerEconomy() {
		if (DEP.vault.exists()) {
			...
			sm.register(Economy.class, new VaultConnector(), this,
	...
	/**
	 * Register an accountholder provider with Gringotts. This is necessary for
	 * Gringotts to find and create account holders of any non-player type.
	 * Registering a provider for the same type twice will overwrite the
	 * previously registered provider.
	 * 
	 * @param type
	 *            type id for an account type
	 * @param provider
	 *            provider for the account type
	 */
	public void registerAccountHolderProvider(String type,
			AccountHolderProvider provider) {
	...
	public FileConfiguration getMessages() {
		...
		if (langStream != null) {
			Reader langReader = new InputStreamReader(getResource(langPath),
	...
	private DAO getDAO() {
		...
		// automatically migrate derby to eBeans if db exists and migration flag
		// hasn't been set
	...
	/**
	 * Some awkward ritual that Bukkit requires to initialize all the DB
	 * classes. Does nothing if they have already been set up.
	 */
